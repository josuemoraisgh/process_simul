...
def create_pbr_material(name, base_color, metallic=0.9, roughness=0.35, alpha=1.0,
                        ...
                        use_noise=False, noise_scale=12.0, noise_strength=0.15):
    """Create a PBR material (Principled BSDF) compatible with glTF 2.0 export.
    
    If use_noise=True, adds a noise texture to modulate roughness for surface
    variation (gives a worn/used look). The noise is baked into vertex colors
    for glTF compatibility later.
    """
...
# Steel materials (more realistic variation)
mat_steel = create_pbr_material('Steel_Brushed', (0.62, 0.64, 0.66), metallic=0.95, roughness=0.28)
mat_steel_dark = create_pbr_material('Steel_Dark', (0.22, 0.24, 0.27), metallic=0.9, roughness=0.45)
mat_steel_worn = create_pbr_material('Steel_Worn', (0.48, 0.44, 0.40), metallic=0.75, roughness=0.55)
mat_steel_painted = create_pbr_material('Steel_Painted', (0.15, 0.30, 0.50), metallic=0.2, roughness=0.6)
mat_refractory = create_pbr_material('Refractory_Brick', (0.30, 0.14, 0.08), metallic=0.0, roughness=0.95)
mat_water = create_pbr_material('Water', (0.10, 0.45, 0.80), metallic=0.0, roughness=0.05, alpha=0.5)
mat_flame_orange = create_pbr_material('Flame_Orange', (1.0, 0.5, 0.0), metallic=0.0, roughness=0.9,
                                       emission_color=(1.0, 0.4, 0.0), emission_strength=8.0)
mat_flame_yellow = create_pbr_material('Flame_Yellow', (1.0, 0.9, 0.3), metallic=0.0, roughness=0.9,
                                       emission_color=(1.0, 0.85, 0.2), emission_strength=6.0)
mat_glass = create_pbr_material('Glass', (0.85, 0.92, 1.0), metallic=0.0, roughness=0.02, alpha=0.25)
mat_copper = create_pbr_material('Copper_Pipe', (0.72, 0.45, 0.2), metallic=0.95, roughness=0.3)
mat_red_sensor = create_pbr_material('Sensor_Red', (0.85, 0.08, 0.08), metallic=0.6, roughness=0.3)
mat_green_sensor = create_pbr_material('Sensor_Green', (0.08, 0.85, 0.15), metallic=0.6, roughness=0.3)
mat_flow_blue = create_pbr_material('Flow_Blue', (0.15, 0.5, 1.0), metallic=0.0, roughness=0.4,
                                    emission_color=(0.15, 0.5, 1.0), emission_strength=1.5, alpha=0.6)
mat_flow_red = create_pbr_material('Flow_Red', (1.0, 0.15, 0.08), metallic=0.0, roughness=0.4,
                                   emission_color=(1.0, 0.15, 0.08), emission_strength=1.5, alpha=0.6)
mat_flow_orange = create_pbr_material('Flow_Orange', (1.0, 0.55, 0.08), metallic=0.0, roughness=0.4,
                                      emission_color=(1.0, 0.55, 0.08), emission_strength=1.5, alpha=0.6)
mat_fan_blade = create_pbr_material('Fan_Blade', (0.45, 0.47, 0.50), metallic=0.92, roughness=0.2)
mat_ground = create_pbr_material('Ground_Concrete', (0.35, 0.33, 0.30), metallic=0.0, roughness=0.85)

# ─── Helper functions ─────────────────────────────────────────────────────────

def apply_bevel(obj, width=0.005, segments=2):
    """Add bevel modifier for soft edges (huge visual impact)."""
...
def apply_auto_smooth(obj, angle=math.radians(40)):
    """Apply auto-smooth normals (compatible with Blender 3.x and 4.x)."""
    try:
        # Blender 4.1+: auto smooth via modifier or mesh attribute
...
def add_vertex_color_variation(obj, base_color, variation=0.05):
    """Add subtle vertex color variation for surface imperfections.
    Vertex colors export to glTF and add realism to flat materials."""
    mesh = obj.data
    ...
    for poly in mesh.polygons:
        for loop_idx in poly.loop_indices:
            r = base_color[0] + random.uniform(-variation, variation)
            g = base_color[1] + random.uniform(-variation, variation)
            b = base_color[2] + random.uniform(-variation, variation)
...
def add_cylinder(name, radius, depth, location, rotation=(0,0,0), segments=32,
                 material=None, bevel=True):
    ...
    if bevel and radius > 0.03:
        apply_bevel(obj, width=min(0.008, radius * 0.08), segments=2)
...
def add_cube(name, size, location, scale=(1,1,1), rotation=(0,0,0),
    ...
    if material:
        obj.data.materials.append(material)
    if bevel:
        apply_bevel(obj, width=min(0.01, size * 0.04), segments=2)
    apply_auto_smooth(obj)
    return obj

def add_sphere(name, radius, location, material=None, segments=24):
...
def create_pipe(name, start, end, radius=0.04, material=None):
    ...
    pipe = add_cylinder(name, radius, length, (cx, cy, cz),
                        rotation=(theta, 0, phi), segments=24,
                        material=material, bevel=False)
    apply_auto_smooth(pipe)
    return pipe
...
# Burner (nozzle with detail)
burner = add_cylinder('Burner', 0.12, 0.3, (-1.1, 0, 0.5),
                       rotation=(0, math.pi/2, 0), segments=32, material=mat_steel)
set_parent(burner, furnace_empty)
# Burner ring
burner_ring = add_torus('Burner_Ring', 0.12, 0.015, (-0.95, 0, 0.5),
                         rotation=(0, math.pi/2, 0), material=mat_steel_dark)
set_parent(burner_ring, furnace_empty)

# Flame (dual-layer for richer emission)
flame_outer = add_sphere('Flame', 0.25, (-0.7, 0, 0.5), material=mat_flame_orange, segments=24)
flame_outer.scale = (2.0, 0.8, 0.8)
bpy.context.view_layer.objects.active = flame_outer
bpy.ops.object.transform_apply(scale=True)
set_parent(flame_outer, furnace_empty)

flame_inner = add_sphere('Flame_Core', 0.15, (-0.6, 0, 0.5), material=mat_flame_yellow, segments=16)
flame_inner.scale = (1.8, 0.5, 0.5)
bpy.context.view_layer.objects.active = flame_inner
bpy.ops.object.transform_apply(scale=True)
set_parent(flame_inner, furnace_empty)
...
steam_drum_body = add_cylinder('SteamDrum_Body', 0.3, 1.8, (0.2, 0, 2.5),
                                ...
                                segments=64, material=mat_steel)
apply_subsurf(steam_drum_body, levels=1)
set_parent(steam_drum_body, steam_drum_empty)

# Flanges on drum ends
for side, sx in [('Left', -0.7), ('Right', 1.1)]:
    flange = add_cylinder(f'SteamDrum_Flange_{side}', 0.35, 0.04,
                           (sx, 0, 2.5), rotation=(0, math.pi/2, 0),
                           segments=48, material=mat_steel_dark)
    set_parent(flange, steam_drum_empty)

# Caps (hemispheres)
for side, sx in [('Left', -0.72), ('Right', 1.12)]:
    cap = add_sphere(f'SteamDrum_Cap_{side}', 0.3, (sx, 0, 2.5),
                     material=mat_steel, segments=32)
    cap.scale = (0.5, 1, 1)
...
mud_drum = add_cylinder('MudDrum', 0.2, 1.4, (0.2, 0, 0.2),
                         rotation=(0, math.pi/2, 0),
                         segments=64, material=mat_steel_worn)
apply_subsurf(mud_drum, levels=1)
set_parent(mud_drum, boiler_root)

# Mud drum flanges
for side, sx in [('Left', -0.5), ('Right', 0.9)]:
    flange = add_cylinder(f'MudDrum_Flange_{side}', 0.24, 0.03,
                           (sx, 0, 0.2), rotation=(0, math.pi/2, 0),
                           segments=48, material=mat_steel_dark)
    set_parent(flange, mud_drum)

# Mud drum caps
for side, sx in [('Left', -0.52), ('Right', 0.92)]:
    cap = add_sphere(f'MudDrum_Cap_{side}', 0.2, (sx, 0, 0.2),
                     material=mat_steel_worn, segments=32)
    cap.scale = (0.5, 1, 1)
    bpy.context.view_layer.objects.active = cap
    bpy.ops.object.transform_apply(scale=True)
...
# Front and back rows of tubes connecting drums (more segments for rounder look)
n_tubes = 12
for i in range(n_tubes):
    x = -0.5 + i * (1.4 / (n_tubes - 1))
    # Front tubes
    tube_f = add_cylinder(f'WWT_Front_{i}', 0.025, 2.0,
                           (x, -0.55, 1.35), segments=16,
                           material=mat_steel, bevel=False)
...
for i in range(n_side):
    ...
    tube_l = add_cylinder(f'WWT_Left_{i}', 0.025, 2.0,
                           (-0.6, y, 1.35), segments=16,
                           material=mat_steel, bevel=False)
    set_parent(tube_l, ww_empty)

# Header pipes at top and bottom (connect tubes to drums)
for label, z_pos, mat in [('Top', 2.35, mat_steel), ('Bottom', 0.35, mat_steel_worn)]:
    hdr_front = add_cylinder(f'Header_{label}_Front', 0.04, 1.5,
                              (0.2, -0.55, z_pos),
                              rotation=(0, math.pi/2, 0), segments=24,
                              material=mat, bevel=False)
    set_parent(hdr_front, ww_empty)
    hdr_back = add_cylinder(f'Header_{label}_Back', 0.04, 1.5,
                             (0.2, 0.55, z_pos),
                             rotation=(0, math.pi/2, 0), segments=24,
                             material=mat, bevel=False)
    set_parent(hdr_back, ww_empty)
...
# Stack / chimney (with flaring top)
stack = add_cylinder('Stack', 0.2, 1.5, (1.5, 0, 3.5),
                      segments=32, material=mat_steel_dark)
set_parent(stack, draft_empty)

# Stack top flare
stack_flare = add_cylinder('Stack_Flare', 0.25, 0.15, (1.5, 0, 4.3),
                            segments=32, material=mat_steel_dark)
set_parent(stack_flare, draft_empty)

# Stack rain cap
stack_cap = add_cylinder('Stack_Cap', 0.3, 0.02, (1.5, 0, 4.42),
                          segments=32, material=mat_steel)
set_parent(stack_cap, draft_empty)
...
# ── Ground Plane (receives shadows in model-viewer) ──────────────────────────
ground = add_cube('GroundPlane', 1.0, (0, 0, -0.55),
                  scale=(5.0, 5.0, 0.02), material=mat_ground, bevel=False)
set_parent(ground, boiler_root)

# Apply all transforms
bpy.ops.object.select_all(action='SELECT')
bpy.ops.object.transform_apply(location=False, rotation=True, scale=True)

# Apply modifiers before export (bevel, subsurf)
...
# Set smooth shading + auto-smooth for all meshes
for obj in bpy.data.objects:
    if obj.type == 'MESH':
        for poly in obj.data.polygons:
            poly.use_smooth = True
        apply_auto_smooth(obj, angle=math.radians(40))
