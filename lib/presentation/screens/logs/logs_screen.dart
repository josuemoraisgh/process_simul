          ...
          ...[
            ...
            return Padding(
              ...
              child: FilterChip(
                ...
                selected: isSelected,
                onSelected: (_) => setState(() => _filterLevel = lv),
                selectedColor: lv == null ? AppColors.primary : _levelColor(lv),
            ...
            : SelectionArea(
                child: Scrollbar(
                  ...
                  child: ListView.builder(
                    ...
                    itemExtent: (_fontSize * 2.5).roundToDouble(),
...
class _LogRow extends StatelessWidget {
  ...
  const _LogRow({required this.entry, required this.index, this.fontSize = 11});
  ...
  Widget build(BuildContext context) {
    final (textColor, bgColor) = switch (entry.level) {
      ...
      LogLevel.debug => (AppColors.textDisabled, Colors.transparent),
      _ => (AppColors.textPrimary, Colors.transparent),
    };

    final isOdd = index.isOdd;

    final rowHeight = (fontSize * 2.5).roundToDouble();
    final badgeFontSize = fontSize - 1;
    final timeWidth = (fontSize * 7.3).roundToDouble();
    final badgeWidth = (fontSize * 2.9).roundToDouble();
    final sourceWidth = (fontSize * 9.1).roundToDouble();

    return Container(
      height: rowHeight,
      color: isOdd ? AppColors.surfaceDark.withValues(alpha: 0.5) : bgColor,
