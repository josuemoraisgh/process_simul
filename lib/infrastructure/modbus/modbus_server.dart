...
class ModbusTcpServer {
  ...
  void _onClient(Socket socket) {
    ...
    socket.listen(
      ...
      onDone: () { _clients.remove(socket); try { socket.destroy(); } catch (_) {} },
      onError: (_) { _clients.remove(socket); try { socket.destroy(); } catch (_) {} },
      cancelOnError: true,
    );
  }
  ...
  void _process(List<int> buf, Socket socket) {
    ...
    while (buf.length >= 8) {
      ...
      final pduLen = (buf[4] << 8) | buf[5]; // includes unit-id + function + data
  ...
  List<int>? _handlePDU(int fn, List<int> data) {
    try {
      switch (fn) {
        case 0x01: return _readBits(data, isInput: false);   // Read Coils
        case 0x02: return _readBits(data, isInput: true);    // Read DI
        case 0x03: return _readRegs(data, isInput: false);   // Read HR
        case 0x04: return _readRegs(data, isInput: true);    // Read IR
        case 0x05: return _writeSingleCoil(data);
        case 0x06: return _writeSingleReg(data);
        case 0x0F: return _writeMultipleCoils(data);
        case 0x10: return _writeMultipleRegs(data);
        default:   return _exception(fn, 0x01); // Illegal function
      }
    } catch (_) {
  ...
  void _sendResponse(Socket socket, int transId, int unitId, int fn, List<int> data) {
